...
**Last updated:** 2026-05-29 (Bundle A + disk persistence + ISEA projection committed (148 tests); ISEA awaits visual verification in studio before JPEG export (3b) begins)
...
- **Tests:** 148 passing under `vitest run` (Phase 8 added 6 in [`packages/world-engine/src/serialize.test.ts`](packages/world-engine/src/serialize.test.ts); Phase 9 + BP 5 added 12 in [`apps/service/src/server.test.ts`](apps/service/src/server.test.ts); post-v0.1.0 added +1 disk-persistence in server.test.ts, +6 ISEA in [`packages/world-renderer/src/isea.test.ts`](packages/world-renderer/src/isea.test.ts)).
...
3. **Snyder ISEA projection + flat-map JPEG export at configurable resolution.** Renderer-only, **standalone deliverable** — not gated on MeridianWorlds merge (see ship-gate auto-memory). Connects to decisions 5, 10, 11, 16. **Split into two sub-deliverables:**
   - **3a. ISEA projection in live view (committed 2026-05-29; visual verification deferred).** New `packages/world-renderer/src/isea.ts` implements gnomonic-barycentric mapping onto the icosahedral 4-row "Traveller wargame" net (5.5 × 3·√3/2 in unit-edge coords, aspect ≈ 2.12:1). 12 vertices, 20 faces, per-face 2D layout coordinates all hardcoded. `iseaForward(lat, lon)` finds the face by max dot-product with face centroids, then computes barycentric within. `iseaForwardOnFace(lat, lon, face)` lets the renderer anchor a whole Voronoi cell to one face so cells don't tear across the net. Within-face transform is approximate equal-area (exact only at vertices and centroid); true Snyder reserved for if precision becomes load-bearing. Wired into `canvas.ts` `makeProjector` + cell-fill loop; panel dropdown gains `isea (flat icosahedron)`. Graticule is **disabled on ISEA in v1** (parallels become zigzags across faces — defer). 6 acceptance tests in `isea.test.ts` cover net extents, pole projection, in-bounds, finite output. **Before 3b:** open studio at 5173, switch projection to ISEA, confirm net layout (5 cap triangles top, equator zigzag strip middle, 5 cap triangles bottom shifted 0.5L right) reads correctly. If wrong, file a redirect entry under §4 and revert/adjust.
   - **3b. JPEG export at chosen resolution (not started).** Default 4096×2048; render to offscreen canvas at the chosen pixel size via the existing renderer, then `canvas.toBlob('image/jpeg', 0.92)` and trigger download. Add a number input + button in the studio Panel under "World file" or a new "Export" section. Letterboxing inside the chosen aspect is automatic (renderer already computes edgePx from `min(W/ISEA_NET_WIDTH, H/ISEA_NET_HEIGHT)`).
...
4. `git status` — if uncommitted work exists, §8 says what state it's in. As of this writing, the ISEA projection (3a) is sitting uncommitted awaiting visual verification.
5. `npm install && npx vitest run` to verify the test suite is still green (currently 148).
6. Run the studio: `cd apps/studio && npx vite` — open `http://127.0.0.1:5173/`. Default N=5000; default projection orthographic with the graticule overlay visible; mode dropdown lists climate / satellite / elevation / temperature / humidity / clouds / rivers / plates / cells / dots (currents UI hidden — see `project_currents_hidden_until_physics_fix` auto-memory). Projection dropdown also offers `isea (flat icosahedron)` if 3a is in tree.
7. Run the service: `npm start --workspace=apps/service` (default port 8787). With `--worlds-dir <path>` (or `WORLDS_DIR` env), worlds persist to disk under that directory and are reloaded on restart.
8. If §5 lists decisions as open, confirm with the user before assuming.
9. **Update this file at the end of any non-trivial session.** New decisions go in §4 with the date.
